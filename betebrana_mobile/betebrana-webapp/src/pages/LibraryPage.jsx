...
export default function LibraryPage() {
  ...
  return (
    <div className="pt-4 md:pt-32 pb-24 px-6 md:px-8 max-w-[1200px] mx-auto relative min-h-screen bg-[#F3F8F6] dark:bg-[#101A16] transition-colors">
        ...
        {TABS.map(({ key, label }) => (
          <button
            ...
            className={`pb-3 text-sm font-bold transition-colors relative ${
              tab === key ? 'text-[#78A090]/90 dark:text-[#A7C7B8]' : 'text-zinc-400 dark:text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'
            }`}
          >
            {label}
            {tab === key && (
              <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#78A090]/85 backdrop-blur-sm dark:bg-[#A7C7B8] rounded-t-full" />
            )}
          </button>
        ))}
      </div>
            ...
            >
              <div className="aspect-[3/4] rounded-2xl overflow-hidden shadow-lg border border-zinc-100 dark:border-zinc-800 mb-4 bg-zinc-100 dark:bg-zinc-800 relative transition-colors">
                ...
                <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex justify-center">
                  <div className="w-full py-2 bg-[#78A090]/85 backdrop-blur-sm text-white text-[10px] uppercase tracking-wider font-bold rounded-lg flex items-center justify-center">
                    {t('Open')}
                  </div>
                </div>
              </div>
              ...
              <span className={`inline-block text-[10px] font-bold px-2.5 py-1 rounded-full ${
                book.status === 'Active Rental'
                  ...
                  ? 'bg-[#EEF5F2] dark:bg-[#20362D]/60 text-[#78A090]/90 dark:text-[#A7C7B8]'
